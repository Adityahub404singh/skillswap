...
// 401 FIX: Token ko clean karke attach kar rahe hain
function getAuthHeader(): HeadersInit | undefined {
  try {
    ...
    if (cleanToken && cleanToken !== "undefined") {
      console.log("DEBUG: Attaching token to header:", cleanToken.substring(0, 10) + "...");
...
export async function customFetch<T = unknown>(
  ...
  // 401 FIX: Merge headers with Auth correctly
